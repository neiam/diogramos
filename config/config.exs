...
# SENTRY_DSN environment variable (read out of the elixir-args secret in
# app.yml). When the DSN is empty, Sentry quietly no-ops.
config :sentry,
  enable_source_code_context: true,
  root_source_code_paths: [File.cwd!()],
  environment_name: Mix.env()

# libcluster topology is configured in runtime.exs (prod-only) so it
# never tries to talk to the k8s API in dev/test.
